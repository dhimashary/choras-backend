...
try:
    # Prefer constrained Delaunay triangulation implemented in our utils
    from app.utils.geometry_utils import triangulate_face_cdt_shapely
except Exception:
...
def detect_cavities(
    faces: Sequence,                                   # List[FaceRecord]
    ...
    *,
    ...
    for fi, f in enumerate(faces):
        ...
        tris_for_face = _triangulate_face_with_fallback(list(f.verts), unique_vertices)
    ...
    for fi, f in enumerate(faces):
        c, n = _polygon_centroid_and_normal(f.verts, unique_vertices)
        centroids[fi] = c
        normals[fi] = n
